...
def get_SDC_rate_perinst():
    ...
    for f in range(0, run_count):
        ...
        if ("hang" in error_msg):
            ...
            if isStandardOutput:
                print("use standard output with index " + str(f))
                file_out = std_output + "/outputfile-" + str(f)
                file_output_txt = "PROOUTPUT"
            else:
                print("use file output with index " + str(f))
