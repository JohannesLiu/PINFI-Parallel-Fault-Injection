...
UINT64 fi_inject_instance0 = 0;
UINT64 fi_inject_instance1 = 0;
...
VOID FI_InjectFault_FlagReg(VOID *ip, UINT32 reg_num, UINT32 jmp_num, CONTEXT *ctxt) {
    ...
    if (fi_iterator == fi_inject_instance0 || fi_iterator == fi_inject_instance1 ) {
        fprintf(stderr, "FI_InjectFault_FlagReg %ld \n", fi_iterator);
        bool isvalid = false;

        const REG reg = reg_map.findInjectReg(reg_num);
        if (REG_valid(reg)) {
            fprintf(stderr, "FI_InjectFault_FlagReg fi counter: %ld \n", fi_counter);
            fprintf(stderr, "FI_InjectFault_FlagReg fi_inject_instance0: %ld \n", fi_inject_instance0);
            fprintf(stderr, "FI_InjectFault_FlagReg fi_inject_instance1: %ld \n", fi_inject_instance1);
...
//PRINT_MESSAGE(4, ("Executing: Valid Reg name %s\n", REG_StringShort(reg).c_str()));
            ...
            if (reg_map.isFloatReg(reg_num)) {
                ...
                fprintf(stderr, "inject_CCS fi counter: %ld \n", fi_counter);
                fprintf(stderr, "inject_CCS fi_inject_instance0: %ld \n", fi_inject_instance0);
                fprintf(stderr, "inject_CCS fi_inject_instance1: %ld \n", fi_inject_instance1);
...
VOID FI_InjectFault_Mem(VOID *ip, VOID *memp, UINT32 size) {
    if (fi_iterator == fi_inject_instance0 || fi_iterator == fi_inject_instance1 ) {
        fprintf(stderr, "FI_InjectFault_Mem %ld \n", fi_iterator);
        if (size == 4) {
        ...
        *(temp_p + byte_num) = *(temp_p + byte_num) ^ (1U << offset_num);
        ...
        fprintf(stderr, "FI_InjectFault_Mem fi counter: %ld \n", fi_counter);
        fprintf(stderr, "FI_InjectFault_Mem fi_inject_instance0: %ld \n", fi_inject_instance0);
        fprintf(stderr, "FI_InjectFault_Mem fi_inject_instance1: %ld \n", fi_inject_instance1);

        fprintf(activationFile, "Activated: Memory injection in %p\n", ip);
        fclose(activationFile); // can crash after this!
        fi_counter += 1;
        if(fi_counter==2){
            activated = 1;
...
VOID get_instance_number(const char *fi_instcount_file) {
    ...
    if (!(fioption.Value() == CCS_INST || fioption.Value() == FP_INST || fioption.Value() == SP_INST ||
          fioption.Value() == ALL_INST)) {
    ...
    fclose(urandom);
    srand(seed);
    fi_inject_instance0 = random() / (RAND_MAX * 1.0) * total_num_inst;
    // fi_inject_instance0 = (genrand64_int64() % total_num_inst - 5) + 1;
    fi_inject_instance1 = fi_inject_instance0 + genrand64_int64() % 6;
    fprintf(stderr, "get_instance_number: \n");
    fprintf(stderr, "fi_inject_instance0: %ld \n", fi_inject_instance0);
    fprintf(stderr, "fi_inject_instance1: %ld \n", fi_inject_instance1);
